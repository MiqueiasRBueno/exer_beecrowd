...
# ela gostaria de saber quantos anos levará para que a cidade menor (sempre é a cidade A) ultrapasse a cidade B
# em população. Claro que ela quer saber apenas para as cidades cuja taxa de crescimento da cidade A é maior do que
# a taxa de crescimento da cidade B, portanto, previamente já separou para você apenas os casos de teste que tem
# a taxa de crescimento maior para a cidade A.
...
# com um digito após o ponto decimal cada,
...
# sobre uma população de 100 pessoas resultará em 102 pessoas, e não 102.5 pessoas, enquanto um crescimento de
# 2.5% sobre uma população de 1000 pessoas resultará em 1025 pessoas. Além disso, não utilize variáveis de
...
# cidade B em número de habitantes. Obs.: se o tempo for mais do que 100 anos o programa deve apresentar a mensagem:
# Mais de 1 seculo. Neste caso, acredito que seja melhor interromper o programa imediatamente após passar de 100 anos,
# caso contrário você poderá receber como resposta da submissão deste problema "Time Limit Exceeded".

numero_de_teste_T = int(input())
for n in range(numero_de_teste_T):
    entradas = input().split()
    entrada_PA, entradas_G1  = int(entradas[0]), float(entradas[2])
    entrada_PB, entradas_G2 = int(entradas[1]), float(entradas[3])
    pa , pb = entrada_PA, entrada_PB
    anos = 0
    while pa <= pb:
        pa += int(pa * entradas_G1 / 100); pb += int(pb * entradas_G2 / 100)
        anos += 1
        if anos > 100:
            break
