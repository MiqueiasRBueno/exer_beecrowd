...
# em sequência (considerando o X nesta soma) para que a soma
# ultrapasse a Z o mínimo possível. Escreva o valor final da contagem.
...
# Como o valor 30 está dentro da especificação (maior do que X) ele será válido e então deve-se processar
...
valor_para_X = int(input())
while True:
    valor_para_Z = int(input())
    if valor_para_Z > valor_para_X:
        break
x = soma = valor_para_X
cont = 1
while soma < valor_para_Z:
    cont += 1
    x += 1
    soma += x
